...
{
    ret_e ret;
	ret = (ret_e)uart_callback_set(uart, uart_cb, NULL);
	
	if (ret==OKAY) 
	{
		ret = (ret_e)uart_rx_enable(uart ,rx_buf,sizeof (rx_buf),RECEIVE_TIMEOUT);
        if (ret!=NOT_OKAY) 
		{
...
{
    ret_e ret;
    ret = (ret_e)uart_tx(uart, tx_buf, len, SYS_FOREVER_US);
	if (ret!=OKAY) 
	{
